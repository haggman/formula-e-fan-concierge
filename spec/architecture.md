...
2. **CX → subagent (REVISED 2026-06-19):** subagent runs in the **Agent Engine** runtime (`adk deploy`); CX reaches it as a tool **discovered through Agent Registry** over MCP (the native path). Fallback: a thin StreamableHttp **MCP server on Cloud Run** (Service Agent ID Token, async) in front of the Agent-Engine agent; then OpenAPI. Agent-as-a-tool is not the backbone (binds the subagent inside CX, Preview). The exact Agent-Engine→CX wire is partly Preview → the spike confirms it. See `spec/cx_integration_spike.md`.
