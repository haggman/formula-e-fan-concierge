...
8. **[CLOSED] `fanbotv2/HD_v02.mp4`.** Viewed — it shows the car/stats, not a fan app; not
   useful. Dropped.
9. **[REVISED 2026-06-19] CX → subagent transport.** Subagent runs in the **Agent Engine** runtime (`adk deploy`); CX reaches it as a tool **discovered through Agent Registry** over MCP — the native, on-platform path (was: a hand-rolled MCP server on Cloud Run, now demoted to fallback). Order: Agent Engine + Registry (primary) → thin Cloud Run MCP facade (fallback) → OpenAPI (second fallback); **Agent-as-a-tool** rejected as backbone (in-CX agent, Preview). The exact Agent-Engine→CX wire is partly Preview, so the spike must confirm it. (`spec/cx_integration_spike.md`.)
...
| 9 | **`frame_tools` + `scorer` re-aim** | parallel-ready | spec'd; implement field-wide + selected-car boost; feeds #4 |
